...
function compact_string_row(x::CartesianIndex{2})
    io = IOContext(IOBuffer(), :compact => true)
    print(io, x.I[1], ",", x.I[2])
    return String(take!(io.io))
end
...
show(io::IO, ::MIME"text/plain", t::Table) = showtable(io, t)
show(io::IO, t::Table) = showtable(io, t)
show(io::IO, ::MIME"text/plain", t::ProductTable) = showtable(io, t)
show(io::IO, t::ProductTable) = showtable(io, t)
function showtable(io::IO, @nospecialize t)
    ...
    print(io, "$typename with $ncols column$(ncols == 1 ? "" : "s") and $nrows row$(nrows == 1 ? "" : "s")")
